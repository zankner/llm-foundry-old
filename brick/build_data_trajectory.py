...
def generate_samples(
    ...
    for batch in data_trajectory:
        for sample_idx in batch:
            ...
            sample = {"tokens": sample["tokens"], "idx": sample["idx"]}
            if sample_idx != sample["idx"]:
                raise ValueError("All sample indices should be the same")
            yield sample

def fetch_data_trajectory(proxy_ckpt):
    # Loading the data trajectory
    with tempfile.NamedTemporaryFile() as tmp_file:
        get_file(os.path.join("oci://mosaicml-internal-checkpoints",proxy_ckpt),
                 tmp_file.name,
                 overwrite=True)
...
if __name__ == "__main__":
    ...
    data_trajectory = fetch_data_trajectory(args.proxy_ckpt)
