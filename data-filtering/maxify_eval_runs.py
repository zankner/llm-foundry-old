...
def compute_averages(run_data, logger_keys, prev_max):
    ...
    for run in runs:
        ...
        try:
            ...
            for eval_step in history:
                ...
                composite_scores = {
                    f'metrics/maxed_model_gauntlet/{k}': v
                    for k, v in composite_scores.items()
                }

                composite_scores[
                    'metrics/maxed_model_gauntlet/category-average'] = sum(
                        composite_scores.values()) / len(
                            composite_scores.values())

                to_log[step] = composite_scores

            new_run = wandb.init(project="amortized-selection",
                                 name=f"maxified-{name}",
                                 group=group,
                                 tags=run.tags)
