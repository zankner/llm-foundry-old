...
for subset_idx, subset in enumerate(PILE_SUBSETS.keys()):
    PILE_SUBSETS[subset]["domain"] = subset_idx
...
def build_hf_dataset(
    ...
    hf_dataset = hf_dataset.filter(
        lambda example: example["meta"]["pile_set_name"] == subset)
...
def main(args: Namespace) -> None:
    ...
    """
    dataset_constants = CONSTS["the_pile"]
    ...
    if args.concat_tokens is not None:
        ...
        raise ValueError("Concat mode bust be true")

    subset_name = args.subset
    subset = PILE_SUBSETS[subset_name]
    for split_name in args.splits:
        ...
        expected_num_samples = subset["raw_samples"]
        truncate_num_samples = split.truncated_samples
        # Only generate the splits requested
        if folder_split not in args.splits:
            continue

        subset_fmt = subset["name"]

        # Get samples
        dataset = build_hf_dataset(dataset_name="EleutherAI/pile",
                                   subset=subset_fmt,
        ...
        # Write samples
        print(f'Converting {folder_split} / {subset_fmt} to MDS format...')
        print(
        ...
        with MDSWriter(columns=columns,
                       out=os.path.join(args.out_root, subset_name,
                                        folder_split),
